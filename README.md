...
- Python 3.11
- FastAPI（`lifespan` 启动 + 依赖挂在 `app.state`，无模块级全局可变状态）
- LangGraph
- FAISS（`IndexFlatL2`）/ Milvus Lite —— 通过 `VECTOR_STORE` 可切换的向量库后端
- PyPDF
...
# 向量库后端：faiss（默认）或 milvus（Milvus Lite，本地）
VECTOR_STORE=faiss

# 可选：embedding 批处理大小与缓存路径（默认值见 app/config.py）
# EMBEDDING_BATCH_SIZE=64
# EMBEDDING_CACHE_PATH=.embedding_cache/embeddings.json
